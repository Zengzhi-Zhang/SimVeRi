...
class BridgeHelper(object):
    ...
    def get_carla_blueprint(sumo_actor, sync_color=False):
        ...
        [SimVeRi v1.1] Support suffixed vtype IDs (for example vehicle.nissan.patrol_hard)
        """
        blueprint_library = BridgeHelper.blueprint_library
        type_id = sumo_actor.type_id
        original_type_id = type_id

        if type_id not in [bp.id for bp in blueprint_library]:
            for suffix in ['_base', '_hard', '_occ']:
                if type_id.endswith(suffix):
                    type_id = type_id[:-len(suffix)]
                    logging.debug('[BridgeHelper] Stripped suffix from %s -> %s', 
                                original_type_id, type_id)
                    break

        if type_id in [bp.id for bp in blueprint_library]:
            blueprint = blueprint_library.filter(type_id)[0]
            logging.debug('[BridgeHelper] sumo vtype %s found in carla blueprints', original_type_id)
        else:
        ...
        logging.debug(
            ...
            \tblueprint: %s
            \tcolor: %s''', original_type_id, blueprint.id,
